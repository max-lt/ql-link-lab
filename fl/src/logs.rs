...
pub fn run(args: Args) -> Result<()> {
    ...
    let mut handle = rusb::open_device_with_vid_pid(PASSPORT_VID, PASSPORT_PID)
        .context("no Passport Prime (1307:0165) -- plugged in and running firmware?")?;
    let config = handle.device().active_config_descriptor().context("reading config descriptor")?;

    // The debug channel is the vendor-specific (class 0xFF) interface's bulk-IN endpoint.
    let mut iface = None;
    let mut ep_in = None;
    ...
    loop {
        ...
        if frame[0] == FRAME_LOG {
            ...
            while let Some(pos) = records.iter().position(|b| *b == RECORD_TERMINATOR) {
                let line =
                    String::from_utf8_lossy(&records[..pos]).trim_end_matches(['\r', '\n']).to_string();
                records.drain(..=pos);
                if line.is_empty() {
                    continue;
                }
